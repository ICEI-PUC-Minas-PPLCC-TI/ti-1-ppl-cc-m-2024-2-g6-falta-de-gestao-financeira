...
async function calcularDados(selectedYear) {
  try {
    ...
    if (gastoPercentual >= 0) {
      rowMargemDeLucro.insertAdjacentHTML(
        "beforeend",
        `% Margem de lucro<div class="right green"> ${(
          margemLucro * 100
        ).toFixed(2)} %<div>`
      );
    } else {
      rowMargemDeLucro.insertAdjacentHTML(
        "beforeend",
        `% Margem de lucro<div class="right red"> ${(margemLucro * 100).toFixed(
          2
        )} %<div>`
      );
    }
...
async function listarCategorias(type, selectedYear) {
  try {
    ...
    const totalGeral = filteredEntries.reduce(
      (acc, entry) => acc + parseFloat(entry.value),
      0
    );
...
function atualizarCategorySummary(categoriesData) {
  ...
  categoriesData.forEach(({ name, total, percent }) => {
    categoryBody.insertAdjacentHTML(
      "beforeend",
      `
        <div>${name}</div>
...
document
  .getElementById("create-entry-form--income")
...
document
  .getElementById("create-entry-form--expense")
  .addEventListener("click", () => {
    const yearDropdown = document.getElementById("yearDropdown");
    const selectedYear = yearDropdown.value || new Date().getFullYear();
